...
- [ ] make use of the ~:dir~ header argument to set the compile output of the code blocks
- [ ] is ~:project-type~ necessary? -> not really, as it is an implicit information based on ~:main~ (and ~:class~)?
- [ ] implement the elisp->csharp parser
