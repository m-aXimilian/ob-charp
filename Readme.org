...
- =:nugetconfig= :: path to a custom /NuGet.config/ file to use for the code block. This is useful if you have a local *.nupkg or a feed different from [[https://www.nuget.org/][nuget.org]] and want to use it in your source block.
